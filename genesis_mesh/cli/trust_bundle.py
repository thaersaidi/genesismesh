...
@click.option(
    ...
    help="Optional review receipt output path.",
)
@click.option(
    "--format",
    "output_format",
...
def load_trust_bundle(path: Path) -> dict[str, Any]:
    """Load a bundle JSON file and translate parse failures into Click errors."""
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except OSError as exc:
        raise click.ClickException(f"Could not read trust bundle: {exc}") from exc
    except json.JSONDecodeError as exc:
        raise click.ClickException(f"Trust bundle is not valid JSON: {exc}") from exc
    if not isinstance(payload, dict):
        raise click.ClickException("Trust bundle JSON must be an object")
    return payload
...
def bundle_summary(bundle: dict[str, Any]) -> dict[str, Any]:
    ...
    connectome_summary = (
        _typed_dict(connectome.get("summary"))
    )
...
def _typed_dict(value: Any) -> dict[str, Any]:
    """Return a JSON object value as a typed dict for static checking."""
...
def validate_bundle_against_review(
    ...
    *,
    ...
    """Validate bundle identity against an already-fetched sovereign review."""
    live_material = {
    ...
    if report["errors"]:
        ...
        raise click.ClickException(f"{label} trust bundle mismatch: {details}")
    return report
...
def _fetch_optional(session: requests.Session, url: str, label: str) -> dict[str, Any]:
    """Fetch optional JSON while preserving status and payload."""
    try:
        response = session.get(url, timeout=10)
    except requests.RequestException as exc:
...
def _contains_forbidden_key(value: Any) -> bool:
    """Return true when a JSON structure contains secret-shaped field names."""
    forbidden = (
        "private_key",
