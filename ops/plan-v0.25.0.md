...
v0.24.0 produces signed trust decisions and TrustEvidence records. v0.25.0 makes
...
- Optional TrustEvidence overlay: when a directory of v0.24.0 TrustEvidence
  files is supplied, Atlas verifies each signature and shows verdicts against
  the edges they reference. Unverifiable records are shown as unverified, never trusted.
...
- (overlay) verified TrustEvidence records and their verdicts

## Success Criteria

- [x] `atlas build` produces a self-contained `atlas.json` + static HTML from a
      graph export, with no network calls at view time.
- [x] The `/atlas` operator-console page renders the live graph read-only.
- [x] Active, expiring, and revoked edges are visually distinct and match the
      Connectome view counts.
- [x] Supplying an evidence directory overlays verified verdicts; tampered or
      foreign-keyed evidence renders as unverified.
- [x] Atlas performs no writes and exposes no mutation path.
- [x] Tests cover static build output, the console surface, and evidence overlay
      verification.
...
- The `genesis-mesh atlas build` command and the read-only `/atlas` console
  surface.
- TrustEvidence-overlay verification reusing v0.24.0 `verify_trust_evidence`.
...
- Requires v0.24.0 `TrustEvidence` + `verify_trust_evidence` for the evidence
  overlay. The graph/Connectome surfaces have no such dependency and could ship
  first if v0.24.0 slips.
